...
if ($total <= 0) { 
    ...
    $consulta = $produtopa->listarPor($tipo,$termo,$valor,$limite,$offset);
    ...
    if(!$consulta){
        ...
        if(!empty($_GET['tipo'])||!empty($_GET['termo'])||!empty($_GET['valor'])){
            $total_res=$produtopa->buscar($termo,$tipo)->num_rows;
        }else{
            $total_res=$total;
        }
