...
?>
<form action="alterarfuncionario.php" method="POST" id="form">
<h1>Alterar Dados do Funcionário</h1>
	...
	<p>Telefone:</p>
	<p><input type="number" name="telefone" id="telefone" size="14" maxlength="14" pattern="\([0-9]{2}\)[0-9]{4,5}-[0-9]{4}" value="<?= $funcionario->getTelefone() ?>"required></p>
	<p>Digite o email:</p>
...
<form action="alterarfuncionario.php" method="POST" id="form">
	<h1>Alterar Senha do Funcionário</h1>
	<input type="hidden" value="<?= $funcionario->getCodFunc() ?>" name="cod_func_senha">
	<p>Nova senha:</p>
	<p><i>*letras e números,@,_,- de 5 a 10 caracteres</i></p>
	<p><input type="password" name="nova_senha" size="10" maxlength="10" pattern="[0-9a-zA-Z@_-]{5,10}" required></p>
	<p>Redigite a nova senha:</p>
	<p><input type="password" name="validar_senha" size="10" maxlength="10" pattern="[0-9a-zA-Z@_-]{5,10}" required></p>
	<p><input type="submit" name="botao_senha" value="Alterar Senha"></p>
</form>
<?php
}
	...
	if (isset($_POST['botao_senha'])) {
		require_once 'persistence/FuncionarioPA.php';
		require_once 'model/Funcionario.php';
		$funcionariopa=new FuncionarioPA();
		$funcionario=new Funcionario();

		if ($_POST['nova_senha'] !== $_POST['validar_senha']) {
			echo "<h2>A senha e a senha redigitada não condizem! <a href='alterarfuncionario.php?cod_func=".$_POST['cod_func_senha']."'>Voltar</a></h2>";
		} else {
			$funcionario->setSenha($_POST['nova_senha']);
			$funcionario->criptografar();
			if ($funcionariopa->alterarSenha($_POST['cod_func_senha'],$funcionario->getSenha())) {
				echo "<h2>Senha do funcionário alterada com sucesso!</h2>";
			}else{
				echo "<h2>Erro na tentativa de alterar a senha! <a href='alterarfuncionario.php?cod_func=".$_POST['cod_func_senha']."'>Alterar</a></h2>";
			}
		}
	}
?>
