...
<form action="recuperarsenha.php" method="POST" id="form">
...
if (isset($_POST['botao'])) {
	...
	if ($_POST['senha'] != $_POST['validar']) {
		...
		$cod_cli=$clientepa->verificarLoginEmail($_POST['login'],$_POST['email']);
		if (!$cod_cli) {
			echo "<h2>Login e email não conferem com nenhum cliente cadastrado!</h2>";
		} else {
			$cliente=new Cliente();
			$cliente->setSenha($_POST['senha']);
			$cliente->criptografar();
			if ($clientepa->redefinirSenha($cod_cli,$cliente->getSenha())) {
